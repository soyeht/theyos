...
| [macos-local-attestation-policy](followup-macos-local-attestation-policy.md) | macOS local enrollment finish remains inert until a server-side platform proof exists, or an explicit Apple Anonymous attestation policy is accepted and implemented with a separate attested challenge/finish path. Platform hints plus UV plus peer-auth are not enough to commit a credential. |
