...
| [macos-local-attestation-policy](followup-macos-local-attestation-policy.md) | A-now selected for macOS local enrollment: build the Apple Anonymous attestation path with separate attested challenge/proof state. Local finish remains inert until the active commit slice stores evidence and commits; request-shaped options plus peer-auth are not enough to commit a credential. |
