...
| [share-relay-hardening-2026-08-05](followup-share-relay-hardening-2026-08-05.md) | Non-blocking follow-up for Share/relay: decide I/O-error byte accounting, preflight one-shot invite compatibility, pin persistent sessions to ClawSite, define any capacity target above the demonstrated 1,000-pair floor, and preserve cache migration behavior in any future runtime. |
| [product-a-per-claw-vpn-t1-pre-e2e-carry-evidence](product-a-per-claw-vpn-t1-pre-e2e-carry-evidence.md) | Non-live evidence checklist for T1 pre-E2E carries: audit sink durability/rotation, fixed audit path, HMAC export redaction plus export policy shape, hardware evidence pack shape, default-off guard, and client-side `IpTunnel` boundary. |
| [t1-iptunnel-dev-client-runner](followup-t1-iptunnel-dev-client-runner.md) | Real T1-T4 dev-host validation still needs a reviewed dev-only `IpTunnel` runner; current tooling validates offer shape, can explicitly open the data-tunnel session seam, and keeps `friend-cli` rejecting `IpTunnel` before connecting. |
| [relay-stream-mesh-ipv6-invalid-hex-placeholder](followup-relay-stream-mesh-ipv6-invalid-hex-placeholder.md) | The `mesh_ipv6` ack placeholder `fd00:c1aw::…` is not valid hex (`w`), so it fails `Ipv6Addr` parsing. The T1 `IpTunnel` server twin (`RelayStreamOfferSession`) was fixed to `fd00:c1a0::…` on this isolated branch; the `household-rs` `GuestCredential` and `friend-cli` twins remain and should be fixed on a shared/main-track branch. LOW/latent — PTY/ClawSite never parse the value today. |
| [t1-orchestration-off-loopback-relay-ack](followup-t1-orchestration-off-loopback-relay-ack.md) | The T1 dev-host orchestration script (`run-t1-dev-datapath.sh`) has no `--allow-public-relay-ack` flag and does not gate a non-loopback `--relay-endpoint` before launch, so the off-loopback (Tailscale-transport) checkpoint isn't executable and could launch-before-fail. The Phase 2/3 runbook is loopback-only until this lands. LOW — loopback flow unaffected. |
