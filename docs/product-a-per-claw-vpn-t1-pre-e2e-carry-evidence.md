...
| Hardware evidence pack shape | `hardware-evidence-pack-validator-tests` | The offline validator accepts only a complete-shaped PR/SHA-bound T1-T4 pack with production excluded, checked T1/T2/T3/T4 items, rollback, owner authorization, and reference content verification; the preflight JSON validator can also follow the private refs with `--check-private-refs`; incomplete packs fail closed without printing values or paths. |
| Audit export policy shape | `audit-export-policy-validator-tests` | The offline validator accepts only a PR/SHA-bound dev-host export policy with `production_activation=false`, reviewed export-key source, key rotation and retention, bounded export data retention, raw subject omission, local pseudonymous hash omission, off-host destination review, and production excluded; the preflight JSON validator also follows `audit_export_policy_ref` with `--check-private-refs`; incomplete or privacy-unsafe policies fail closed without printing values or paths. |
| Device-side session config shape | `device-session-config-validator-tests` | The offline validator accepts only a reviewed Device-side config shape with dev-host/no-production scope, Device-side local role, private IPv4 pair present but not printed, single-claw `/32` route scope, and bounded MTU; the preflight JSON validator also follows `device_session_config_ref` with `--check-private-refs`; invalid configs fail closed without printing private paths or IPv4 values. |
| Private gate status shape | `private-gate-status-tests` | The offline status helper reports only field names for missing or invalid owner/rollback/hardware/audit-export/device-session-config refs, can chain the private validators, and never prints private paths or values. |
...
- export key source, rotation, retention, and privacy policy for any off-host
  ...
  or the preflight record `--check-private-refs` chain used as a shape/privacy
  check before content review;
- reviewed Device-side session config artifact, with
  `scripts/validate-t1-device-session-config.py <config>` or the preflight
  record `--check-private-refs` chain used as a shape/privacy check before
  content review and before any route/interface work;
