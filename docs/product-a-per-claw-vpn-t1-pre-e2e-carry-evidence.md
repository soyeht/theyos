...
| Client-side dev-runner boundary | `rejects_iptunnel_before_connecting` in `friend-cli-rs` and `t1-iptunnel-dev-runner-rs` tests | `friend-cli` still rejects `IpTunnel` before connecting, while the dev runner accepts only member-scoped offers and requires explicit dev-host acknowledgement before the session-open seam. The runner still has no local TUN/utun, route install, packet-pump, or production-app control path; any target open against an activated dev host remains under the #281 dev-host gate. |
