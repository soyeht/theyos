...
| Private gate status shape | `private-gate-status-tests` | The offline status helper reports only field names for missing or invalid owner/rollback/hardware/audit-export refs, can chain the private validators, and never prints private paths or values. |
