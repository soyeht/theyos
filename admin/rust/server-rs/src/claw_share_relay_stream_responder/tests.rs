...
use crate::claw_share_relay_stream_test_support::{
    ...
    now_unix, owner_pub, owner_signer, relay_stream_household_record, relay_stream_machine_cert,
    relay_stream_offer, relay_stream_offer_signed_by, relay_stream_responder_params as params,
...
fn controlled_live_now(
    ...
    Arc::new(move |clock| {
        ...
        match failure {
            LiveClockFailure::Unusable => clock.live_at_for_test(None, Duration::from_millis(1)),
...
async fn relay_stream_responder_device_live_clock_failures_tear_down_real_serve() {
    ...
    .enumerate()
    {
        timeout(Duration::from_secs(5), async {
            ...
            let offer =
                device_offer_expiring_at(0x76 + u8::try_from(index).unwrap(), &keypair, not_after);
    ...
    timeout(Duration::from_secs(2), async {
        ...
        assert!(matches!(
            ...
            Err(RelayStreamResponderError::Noise(
                RelayStreamNoiseError::Contract(RelayStreamContractError::IssuerUnauthorized(_))
            ))
        ));
    })
    .await
