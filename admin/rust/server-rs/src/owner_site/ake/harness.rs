...
use household_rs::{
    MachineCert, MemberDeviceBinding, P256PublicKey, P256Signature, Platform, derive_household_id,
};
...
use crate::owner_site::authority::{
    ...
    OwnerSiteBindingDigest, OwnerSiteBindingId, OwnerSiteChannelAuthKey, OwnerSiteChannelAuthKeyId,
    OwnerSiteMembershipRole, OwnerSiteRemotePrincipal, OwnerSiteResolvedBinding,
    OwnerSiteRevocationTombstone, OwnerSiteRosterBinding, OwnerSiteRosterScope,
...
impl OwnerSiteAkeHarness {
    pub(crate) fn fixture_for_harness(claw_name: &str) -> AkeResult<OwnerSiteAkeFixture> {
        ...
        let member_device =
            MemberDeviceBinding::sign(&member, device.public(), participant_npub.clone(), A2_NOW)
                .map_err(|_| OwnerSiteAkeFailure::Rejected)?;
        ...
        .map_err(|_| OwnerSiteAkeFailure::Rejected)?;
        let scope =
            OwnerSiteRosterScope::injected_for_harness(intent.household_id(), intent.network_id())
                .map_err(|_| OwnerSiteAkeFailure::Rejected)?;
        ...
        .map_err(|_| OwnerSiteAkeFailure::Rejected)?;
        let revoked_generation = OwnerSiteAuthorityGeneration::injected_for_harness(2, [0x42; 32])
            .map_err(|_| OwnerSiteAkeFailure::Rejected)?;
    ...
    pub(super) async fn serve(
        ...
        let c3_accepted =
            matches!(next_a2_binary(&mut socket).await, Some(c3) if pending.accept_c3(&c3).is_ok());
    ...
    fn begin_m1(
        ...
        let mut preview = responder_with_channel_keys(&static_private, &*engine_ephemeral_secret)?;
        let mut plaintext = vec![0u8; MAX_A2_FRAME_BYTES];
...
impl OwnerSiteAkeResponderSession {
    fn accept_m3(&mut self, harness: &OwnerSiteAkeHarness, bytes: &[u8]) -> AkeResult<()> {
        ...
        harness
            ...
            .fetch_add(1, Ordering::SeqCst);
        let h_final = final_handshake_hash(&self.handshake)?;
        let channel_binding = channel_binding(h_final, &self.m2.channel_id, self.m2.channel_epoch)?;
...
impl A2S2Plain {
    fn validate_for(&self, context: &A2RecordContext, sequence: u64, now: u64) -> AkeResult<()> {
...
impl ClientHelloCore {
    fn from_intent(intent: &OwnerSiteIntent, binding_id: OwnerSiteBindingId) -> AkeResult<Self> {
        Ok(Self {
...
{
    let decoded = cbor::from_canonical_slice(bytes).map_err(|_| OwnerSiteAkeFailure::Rejected)?;
...
fn verify_p256_signature(key: &P256PublicKey, digest: &[u8; 32], raw: &[u8]) -> AkeResult<()> {
    ...
    let signature = P256Signature::from_bytes(raw).map_err(|_| OwnerSiteAkeFailure::Rejected)?;
...
mod tests {
    ...
    fn independent_transport_pair() -> (TransportState, TransportState) {
        ...
        exchange(
            &mut device,
            &mut engine,
            encode_canonical(&(A2_DOMAIN, A2_VERSION, "independent-m1")).expect("independent M1"),
            &mut plaintext,
        );
        exchange(
            &mut engine,
            &mut device,
            encode_canonical(&(A2_DOMAIN, A2_VERSION, "independent-m2")).expect("independent M2"),
            &mut plaintext,
        );
        exchange(
            &mut device,
            &mut engine,
            encode_canonical(&(A2_DOMAIN, A2_VERSION, "independent-m3")).expect("independent M3"),
            &mut plaintext,
        );
        (
            device
    ...
    fn assert_authenticated_s2_context_mutation_is_rejected(
        ...
        let received = open_a2_record(&mut device, &s2_wire, 0).expect("S2 tag still authentic");
    ...
    fn assert_authenticated_c3_context_mutation_is_rejected(
        ...
        let mut altered = expected.clone();
        mutate(&mut altered);
        let c3_payload = encode_canonical(&altered.c3_plain(0, hs2)).expect("altered C3 plaintext");
        let c3_wire =
    ...
    fn make_authenticated_c3(
        ...
        let s2_payload = encode_canonical(&context.s2_plain(s2_sequence)).expect("canonical S2");
        let s2_wire = seal_a2_record(engine, &s2_payload, s2_sequence).expect("authenticated S2");
        let s2_sequence = device.receiving_nonce();
        let s2_payload = open_a2_record(device, &s2_wire, s2_sequence).expect("S2 inverse open");
    ...
    fn a2_r1_records_fail_closed_for_tamper_replay_direction_and_context_swaps() {
        ...
        let s2_for_c3_payload = open_a2_record(&mut c3_device, &c3_s2_wire, 0).expect("C3 S2 open");
        let s2_for_c3: A2S2Plain = decode_canonical(&s2_for_c3_payload).expect("C3 S2 canonical");
        s2_for_c3
        ...
        let client_ack: A2C3Plain = decode_canonical(&client_ack_payload).expect("C3 canonical");
        assert!(
            client_ack
    ...
    fn a2_r1_rejects_raw_noncanonical_oversize_and_c3_before_s2() {
        ...
        let raw_plaintext = encode_canonical(&raw_context.s2_plain(0)).expect("raw S2 plaintext");
        assert!(
        ...
        let (mut oversized_cipher_device, _oversized_cipher_engine, _, _) = frozen_transport_pair();
        ...
        let (mut bounded_device, mut bounded_engine, _bounded_context, _) = frozen_transport_pair();
    ...
    fn a2_r1_prologue_swap_fails_before_an_authenticated_m3() {
        ...
        let mut wrong_engine = a2_noise_builder(&altered_prologue)
            ...
            .expect("engine responder");
        let payload =
            encode_canonical(&(A2_DOMAIN, A2_VERSION, "fixture-m1")).expect("canonical payload");
        let mut m1 = vec![0u8; MAX_A2_FRAME_BYTES];
