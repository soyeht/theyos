...
use crate::claw_share_relay_stream_test_support::{
    DATA_TUNNEL_SLOT, RELAY_STREAM_CLAW_ID, attacker_signer, guest_pub, guest_signer, owner_signer,
    relay_stream_household_record, relay_stream_issuer_trust, relay_stream_machine_cert,
    rendezvous_token,
};
...
async fn open_error<P, S, I>(binding: &RelayStreamReverseConnectBinding<P, S, I>) -> DataTunnelError
where
    P: ClawTargetRouter,
    S: ClawTargetRouter,
    I: RelayStreamIpTunnelRouter,
...
async fn binding_routes_pty_offer_to_pty_subrouter() {
    let binding = binding_for(RelayStreamResource::Pty, &owner_signer(), consumed_slots()).await;
