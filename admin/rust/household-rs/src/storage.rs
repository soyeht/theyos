...
pub fn phase3_recovery_manifest_exists(state_dir: &Path) -> bool {
    ...
    //
    // `Ok(_)` and `Err(_)` share a body but are NOT the same decision: one is
    // "evidence exists", the other is "we could not tell, so assume it does".
    // Collapsing them, as `match_same_arms` suggests, would erase the second
    // meaning and make a future author read the fail-closed default as an
    // accident. They are also separated by the guarded `NotFound` arm, so
