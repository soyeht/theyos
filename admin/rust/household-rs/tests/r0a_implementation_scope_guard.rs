...
fn r0a_d2a_tests_stay_inline_and_off_the_cargo_target_ratchet() {
    ...
    // pins the workspace target inventory at 199 (149 of them tests) in this
    ...
    // `claw-share-bridge-rs` workspace member (which is the +1 bin). Both
    // increments are needed to reach 199 — naming only one of them is how the
    // total and its explanation drift apart. D2a still keeps its focused tests
    ...
    for absent in [
        ...
        assert!(
            !package_dir().join(absent).exists(),
            "D2a adds no Cargo test target; R1a.7 pins 199/149 and none of it \
             is D2a's: {absent}"
        );
    }
    ...
    let ratchet_path = ["owner_mesh_", "rendezvous_codec.rs"].concat();
    let r1a7 = read(package_dir().join("tests").join(ratchet_path));
    assert!(r1a7.contains("assert_eq!(targets.len(), 199"));
    assert!(
