...
//!   1. `$OUT_DIR/generated_manifest.rs` — compiled-in Rust const array
...
fn main() {
    ...
    // ── Generate $OUT_DIR/generated_manifest.rs ─────────────────────────
    ...
    let generated_path = PathBuf::from(
        std::env::var_os("OUT_DIR").expect("Cargo must provide OUT_DIR to core-rs/build.rs"),
    )
    .join("generated_manifest.rs");
