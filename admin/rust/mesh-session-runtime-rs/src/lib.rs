...
//!
//! Explicitly NOT in this round's scope: `D1Admission`,
//! `ActiveGateAuthorization`, `RetainedGenerationResolver` real adapters.
//! `household-rs`'s own `SealedBinding::from_membership_key` (feature
//! `mesh-session-runtime` on that crate) is a separate, already-landed
//! piece this facade will eventually call into, not duplicated here.
//!
